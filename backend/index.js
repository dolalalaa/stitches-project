...
app.post("/order/place", async (req, res) => {
  try {
    ...
    const newOrder = new Order({
      items: [...cart],
      totalPrice,
      status: req.body.source === "design" ? "Pending" : "Placed",
      source: req.body.source || "product",
    });
    await newOrder.save();
...
// ════════════════════════════════════════════════════════════════
//  MANNEQUIN — calls Python microservice on port 5000
// ════════════════════════════════════════════════════════════════
const fetch = (...args) =>
  import("node-fetch").then(({ default: f }) => f(...args));

// POST /mannequin/generate
// Receives measurements from frontend, forwards to Python microservice
  ...
  try {
    ...
    if (!chest || !waist || !hip || !armLength) {
      return res.status(400).json({
        success: false,
        message: "Required: chest, waist, hip, armLength (shoulder optional)",
      });
    }
    ...
    res.status(500).json({
      success: false,
      message:
        "Could not connect to mannequin service. Make sure it is running on port 5001.",
      error: error.message,
    });
  }
});

// GET /mannequin/file/:mannequinId
// Fetches the .glb file from Python microservice and streams it to frontend
app.get("/mannequin/file/:mannequinId", async (req, res) => {
  try {
    const { mannequinId } = req.params;
    const response = await fetch(
      `http://localhost:5001/mannequin/${mannequinId}`
    );

    if (!response.ok) {
      return res
        .status(404)
        .json({ success: false, message: "Mannequin file not found." });
    }

    res.setHeader("Content-Type", "model/gltf-binary");
    res.setHeader(
      "Content-Disposition",
      `inline; filename="mannequin_${mannequinId}.glb"`
    );
    response.body.pipe(res);
  } catch (error) {
    res.status(500).json({
      success: false,
      message: "Could not fetch mannequin file.",
      error: error.message,
    });
  }
});
